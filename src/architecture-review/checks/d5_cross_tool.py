...
def _check_envelope_reference(tools: list[str], project_root: Path) -> list[Finding]:
    """Check that all tool schemas include the 8 standard envelope fields."""
    findings: list[Finding] = []
    for tool in tools:
        ...
        all_fields = props | required
        missing = REQUIRED_ENVELOPE_FIELDS - all_fields
        if missing:
            findings.append(Finding(
                severity="warning",
                rule_id="ENVELOPE_REFERENCE",
                message=f"Tool '{tool}' schema missing envelope fields: {sorted(missing)}",
