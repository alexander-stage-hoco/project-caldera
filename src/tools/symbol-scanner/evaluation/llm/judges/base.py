"""Base judge class for LLM-as-a-Judge evaluation of symbol-scanner outputs.

This module re-exports the shared BaseJudge and JudgeResult classes,
and provides symbol-scanner-specific extensions.
"""
...
class BaseJudge(SharedBaseJudge):
    """Symbol-scanner-specific base judge with output resolution.

    Extends the shared BaseJudge with symbol-scanner-specific functionality:
    - Automatic output directory resolution
    - Tool output loading with envelope unwrapping
    - Test mode support via SYMBOL_SCANNER_TEST_MODE env var
    """
    ...
    def __init__(
        ...
        """Initialize the symbol-scanner judge.
    ...
    def invoke_claude(self, prompt: str) -> str:
        ...
        Overrides parent to add SYMBOL_SCANNER_TEST_MODE support for testing.
        """
