"""Unit tests for tree-sitter Python extractor."""
...
from extractors.treesitter_extractor import TreeSitterExtractor
...
class TestTreeSitterExtractor:
    ...
    def test_syntax_error_handling(self, extractor, temp_dir):
        ...
        # Tree-sitter records errors but still extracts what it can
        assert len(result.errors) == 1
        assert result.errors[0]["code"] == "SYNTAX_ERROR"
        assert result.errors[0]["recoverable"] is True

        # Tree-sitter should still extract partial results
        # The 'broken' function might or might not be extracted depending on error recovery

    def test_partial_extraction_on_error(self, extractor, temp_dir):
...
'''
        ...
        # Should still extract valid functions (advantage over AST)
        names = {s.symbol_name for s in result.symbols}
...
'''
    ...
    def test_multiline_docstring(self, extractor, temp_dir):
...
'''
        ...
        result = extractor.extract_file(test_file, "docstring.py")

        assert len(result.symbols) == 1
        func = result.symbols[0]
        assert func.docstring is not None
        assert "multiline docstring" in func.docstring
        assert "multiple paragraphs" in func.docstring

    def test_relative_imports(self, extractor, temp_dir):
        """Test extraction of relative imports."""
        code = '''
from . import sibling
from .. import parent
from ..utils import helper
from ...deep import nested
'''
        test_file = temp_dir / "relative.py"
        test_file.write_text(code)

        result = extractor.extract_file(test_file, "relative.py")

        import_paths = {i.imported_path for i in result.imports}
        assert "." in import_paths or ".sibling" in import_paths or any(
            p.startswith(".") for p in import_paths
        )

    def test_method_calls(self, extractor, temp_dir):
        """Test extraction of method calls."""
        code = '''
def process():
    result = obj.method()
    data = obj.nested.call()
    return result
'''
        test_file = temp_dir / "method_calls.py"
        test_file.write_text(code)

        result = extractor.extract_file(test_file, "method_calls.py")

        # Should find dynamic calls
        dynamic_calls = [c for c in result.calls if c.call_type == "dynamic"]
        assert len(dynamic_calls) >= 2

        callee_symbols = {c.callee_symbol for c in dynamic_calls}
        assert "method" in callee_symbols
        assert "call" in callee_symbols

    def test_class_docstring(self, extractor, temp_dir):
...
class MyClass:
    ...
    def method(self):
        pass
'''
        test_file = temp_dir / "class_doc.py"
        test_file.write_text(code)

        result = extractor.extract_file(test_file, "class_doc.py")

        cls = next(s for s in result.symbols if s.symbol_type == "class")
        assert cls.docstring == "This is the class docstring."
...
def func2():
    pass
'''
        test_file = temp_dir / "lines.py"
        test_file.write_text(code)

        result = extractor.extract_file(test_file, "lines.py")

        symbols_by_name = {s.symbol_name: s for s in result.symbols}

        # func1 starts at line 1
        assert symbols_by_name["func1"].line_start == 1
        assert symbols_by_name["func1"].line_end == 2

        # func2 starts at line 4
        assert symbols_by_name["func2"].line_start == 4
        assert symbols_by_name["func2"].line_end == 5
...
class TestVariableExtraction:
    ...
    def test_dunder_variables_skipped(self, extractor, temp_dir):
        ...
        variable_symbols = [s for s in result.symbols if s.symbol_type == "variable"]
        assert len(variable_symbols) == 1
        assert variable_symbols[0].symbol_name == "VERSION"
    ...
    def test_variable_line_numbers(self, extractor, temp_dir):
        ...
        variable_symbols = [s for s in result.symbols if s.symbol_type == "variable"]
        symbols_by_name = {s.symbol_name: s for s in variable_symbols}
    ...
    def test_annotated_without_value(self, extractor, temp_dir):
        ...
        We only extract when there's an actual assignment with a value.
        """
        code = "x: int\nVAR: str = 'hello'"
        test_file = temp_dir / "test.py"
        test_file.write_text(code)
