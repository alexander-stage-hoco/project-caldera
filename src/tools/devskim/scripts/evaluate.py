#!/usr/bin/env python3
"""
Programmatic evaluation for DevSkim security analysis.
...
def generate_scorecard_json(report: EvaluationReport) -> dict:
    ...
    return {
        ...
        "description": "Evaluation scorecard for DevSkim security analysis",
        "summary": {
...
def run_evaluation(
    ...
    # Run coverage checks
    coverage_checks = run_coverage_checks(analysis, ground_truth_dir=ground_truth_dir)
...
def print_report(report: EvaluationReport):
    ...
    print(c("  DEVSKIM EVALUATION REPORT", Colors.CYAN, Colors.BOLD))
    print(c("=" * 70, Colors.CYAN))
    print()
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate DevSkim security analysis against ground truth"
    )
    parser.add_argument(
        "--analysis", "-a",
        default=None,
        help="Path to analysis JSON file (auto-detects latest if not specified)",
    )
    parser.add_argument(
        "--ground-truth", "-g",
    ...
    parser.add_argument(
        "--output", "-o",
        default="evaluation/results/evaluation_report.json",
    ...
    # Auto-detect analysis path if not specified
    analysis_path = args.analysis
    if analysis_path is None:
        # Find the most recent output.json in outputs/*/
        outputs_dir = script_dir / "outputs"
        if outputs_dir.exists():
            output_files = sorted(
                outputs_dir.glob("*/output.json"),
                key=lambda p: p.stat().st_mtime,
                reverse=True,
            )
            if output_files:
                analysis_path = str(output_files[0])
            else:
                print(f"Error: No output.json files found in {outputs_dir}", file=sys.stderr)
                sys.exit(1)
        else:
            print(f"Error: Outputs directory not found: {outputs_dir}", file=sys.stderr)
            sys.exit(1)
    elif not os.path.isabs(analysis_path):
        analysis_path = str(script_dir / analysis_path)

    ground_truth_dir = args.ground_truth
    if not os.path.isabs(ground_truth_dir):
        ground_truth_dir = str(script_dir / ground_truth_dir)
    ...
    if not Path(ground_truth_dir).is_dir():
        print(f"Warning: Ground truth directory not found: {ground_truth_dir}", file=sys.stderr)
        # Create empty ground truth directory
