"""LLM Evaluation Orchestrator for git-fame.

Coordinates multiple LLM judges to produce a comprehensive qualitative
evaluation of git-fame authorship analysis outputs.
"""
...
class LLMEvaluator(LLMEvaluatorBase):
    """Orchestrates LLM-based evaluation of git-fame outputs.

    Coordinates six specialized judges, each evaluating a specific
    dimension of authorship analysis quality:
    - authorship_quality (25%): Author attribution accuracy
    - bus_factor_accuracy (20%): Bus factor calculation correctness
    - concentration_metrics (20%): HHI and concentration metrics accuracy
    - evidence_quality (15%): Supporting data completeness
    - integration_readiness (10%): SoT Engine compatibility
    - output_completeness (10%): Overall output completeness
    """
    ...
    def register_focused_judges(self) -> None:
        ...
        Focuses on authorship quality and bus factor accuracy (45% of total weight).
        """
        judges = [
            AuthorshipQualityJudge(
                model=self.model,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run LLM evaluation for git-fame")
    parser.add_argument(
        "results_dir",
        type=Path,
        nargs="?",
    ...
    parser.add_argument(
        "--focused",
        action="store_true",
        help="Run focused evaluation (2 judges instead of 6)",
    )
    parser.add_argument(
        "--markdown",
        action="store_true",
    ...
    print("LLM Evaluation for git-fame")
    ...
    if args.focused:
        ...
        print("Running full evaluation (6 judges)...")
    ...
    if args.programmatic_results and args.programmatic_results.exists():
        ...
        # Use classification (not decision) and summary.score (0-1 scale)
        result.programmatic_input = ProgrammaticInput(
            file=str(args.programmatic_results),
            decision=prog_data.get("classification", "UNKNOWN"),
            score=summary.get("score", 0.0),
            checks_passed=summary.get("passed", 0),
            checks_failed=summary.get("failed", 0),
        )
        # Compute combined score (programmatic summary.score is 0-1, convert to 0-5)
        prog_score = summary.get("score", 0.0) * 5.0
    ...
    if args.markdown:
        md_report = evaluator.generate_markdown_report(result, "Git-Fame LLM Evaluation Report")
