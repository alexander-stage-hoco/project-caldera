"""LLM Evaluation Orchestrator for Lizard PoC.

Coordinates multiple LLM judges to produce a comprehensive qualitative
evaluation of Lizard function complexity analysis outputs.
"""
...
class LLMEvaluator(LLMEvaluatorBase):
    """Orchestrates LLM-based evaluation of Lizard outputs.

    Coordinates multiple specialized judges, each evaluating a specific
    dimension of function complexity analysis quality.
    """
    ...
    def __init__(
        ...
        self.analysis_path = analysis_path or self.working_dir / "evaluation" / "results" / "output.json"
    ...
    def register_all_judges(self) -> None:
        """Register all 4 Lizard judges."""
        from .judges.ccn_accuracy import CCNAccuracyJudge
        from .judges.function_detection import FunctionDetectionJudge
        from .judges.statistics import StatisticsJudge
        from .judges.hotspot_ranking import HotspotRankingJudge

        judges = [
            CCNAccuracyJudge(model=self.model, working_dir=self.working_dir, analysis_path=self.analysis_path),
            FunctionDetectionJudge(model=self.model, working_dir=self.working_dir, analysis_path=self.analysis_path),
            StatisticsJudge(model=self.model, working_dir=self.working_dir, analysis_path=self.analysis_path),
            HotspotRankingJudge(model=self.model, working_dir=self.working_dir, analysis_path=self.analysis_path),
        ]
    ...
    def register_judges_by_name(self, names: list[str]) -> None:
        ...
        judge_map = {
            "ccn_accuracy": CCNAccuracyJudge,
            "function_detection": FunctionDetectionJudge,
            "statistics": StatisticsJudge,
            "hotspot_ranking": HotspotRankingJudge,
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run LLM evaluation on Lizard analysis outputs"
    )
    parser.add_argument(
        "--working-dir",
        type=Path,
    ...
    parser.add_argument(
        "--focused",
        action="store_true",
        help="Run only focused judges (CCN accuracy + statistics)",
    )
    parser.add_argument(
        "--judges",
        type=str,
        help="Comma-separated list of judge names to run (e.g., ccn_accuracy,statistics)",
    )
    parser.add_argument(
        "--no-assertions",
        action="store_true",
    ...
    if analysis_path is None:
        analysis_path = args.working_dir / "evaluation" / "results" / "output.json"
    ...
    # Run evaluation
    print(f"\n{'='*60}")
    print("LLM Evaluation - Lizard Function Complexity Analysis")
    print(f"{'='*60}\n")
    ...
    report = evaluator.generate_markdown_report(result, "LLM Evaluation Report - Lizard Function Analysis")
