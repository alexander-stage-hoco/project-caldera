"""End-to-end integration tests for git-blame-scanner.
...
def synthetic_repo(tool_root: Path) -> Path:
    ...
    Uses the single-author synthetic repo as it's a proper git repo.
    """
    return tool_root / "eval-repos" / "synthetic" / "single-author"
...
def test_full_pipeline(tool_root: Path, synthetic_repo: Path):
    ...
    Requires: Run `python scripts/build_repos.py` to create synthetic repos.
    """
    # Skip if synthetic repo doesn't exist or isn't a git repo
    if not synthetic_repo.exists():
        pytest.skip("Synthetic repo doesn't exist - run scripts/build_repos.py first")

    if not (synthetic_repo / ".git").exists():
        pytest.skip("Synthetic repo is not a git repository - run scripts/build_repos.py first")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert output["metadata"]["tool_name"] == "git-blame-scanner"
