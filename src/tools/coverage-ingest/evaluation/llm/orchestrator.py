#!/usr/bin/env python3
"""
Run LLM judges for coverage-ingest evaluation.
...
Usage:
    ...
    python -m evaluation.llm.orchestrator outputs/<run-id> --judge risk_tier_quality
"""
...
def run_all_judges(
    ...
    # Go up 3 levels to reach the coverage-ingest tool directory
...
def print_report(results: dict[str, Any]) -> None:
    ...
    print("\n" + "=" * 70)
    print("COVERAGE-INGEST LLM EVALUATION REPORT")
    print("=" * 70)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run LLM judges for coverage-ingest evaluation"
    )
    parser.add_argument(
        "output_dir",
        help="Path to output directory containing coverage analysis results",
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    parser.add_argument(
        ...
        type=Path,
        help="Path to programmatic evaluation JSON (evaluation_results.json)",
    )
    ...
    if args.programmatic_results and args.programmatic_results.exists():
        ...
        programmatic_input = ProgrammaticInput(
            ...
            checks_passed=prog_data.get("passed", 0),
            checks_failed=prog_data.get("total", 0) - prog_data.get("passed", 0),
        )
