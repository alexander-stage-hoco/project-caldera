...
The evaluation framework assesses scc's fitness for integration into the DD Platform MVP across 10 dimensions with 66 individual checks.
...
| v2.1 (Current) | 10 | 66 | 70% | 30% |

### Key Changes in v2.1

1. **New Coverage Checks:**
   - CV-10: Python docstrings counted as comments
   - CV-11: Multi-line string handling validation

2. **New Reliability Check:**
   - RL-9: Semantic consistency between runs (cross-run comparison)

### Key Changes in v2.0

1. **New Dimensions Added:**
   - Per-File Analysis (PF-1 to PF-6) - 10% weight
   - Directory Analysis (DA-1 to DA-5) - 10% weight
...
| RL-9 | Semantic Consistency | Semantic comparison between runs shows no metric changes | No differences in Code, Comment, Blank, Lines, Complexity, or Count per language |

**RL-9 Details:**

Unlike RL-6 which compares output hashes, RL-9 performs a semantic comparison that identifies specific metrics that changed between runs. This provides more actionable diagnostic information when consistency issues are detected.

The check compares:
- Language presence (added/removed languages)
- Per-language metrics: Code, Comment, Blank, Lines, Complexity, Count

**Acceptance Criteria:**
- Zero differences between current and previous run
- All languages present in both runs
- All metrics identical for each language

**Edge Cases:**
- If no previous run is available, the check runs scc twice and compares the outputs
- Tolerates minor timestamp differences in provenance (not compared)
...
Validates language detection, file counting, and language-specific parsing behavior.
...
| CV-10 | Python Docstrings as Comments | Python docstrings counted in Comment column | Python files with docstrings have `Comment > 0` |
| CV-11 | Multi-line String Handling | Multi-line strings do not cause incorrect line counts | `Lines` approximately equals `Code + Comment + Blank` |

**CV-10 Details:**

Python docstrings (triple-quoted strings used for documentation) should be counted as comments, not code. This check validates that scc correctly categorizes docstrings in the Comment column.

**Acceptance Criteria:**
- Python files containing docstrings must have `Comment > 0`
- Files with known docstring content (e.g., files with "docstring" in the name) should show comments

**Edge Cases:**
- Docstrings used as regular string literals (not documentation) may be counted as code
- Files with only docstrings and no executable code should have `Code = 0` and `Comment > 0`

**CV-11 Details:**

Multi-line strings across languages (template literals in JS/TS, verbatim strings in C#, raw strings in Python/Go/Rust) should not cause incorrect line counting.

**Acceptance Criteria:**
- `Lines` should approximately equal `Code + Comment + Blank` (within 20% tolerance)
- Files with multi-line string content should have `Lines >= Code`

**Edge Cases:**
- Template literals with embedded expressions may have complex line counting
- Heredocs and multi-line raw strings spanning many lines
- Tolerance of 20% variance allowed for edge cases
...
| Scenario | Expected Comment Count | Notes |
|----------|------------------------|-------|
| Module-level docstring | > 0 | Triple-quoted string at module start |
| Function docstring | > 0 | Triple-quoted string after function def |
| Class docstring | > 0 | Triple-quoted string after class def |
| Docstring as string literal | May be 0 | Assignment to variable may count as code |

### Multi-line String Tolerance

| Language | String Type | Tolerance |
|----------|-------------|-----------|
| Python | Triple-quoted strings, f-strings | 20% variance |
| JavaScript/TypeScript | Template literals | 20% variance |
| C# | Verbatim strings (@"...") | 20% variance |
| Go | Raw strings (backticks) | 20% variance |
| Rust | Raw strings (r#"..."#) | 20% variance |
