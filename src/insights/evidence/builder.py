...
class EvidenceRegistryBuilder:
    ...
    def __init__(
        ...
        """
        Initialize the EvidenceRegistryBuilder with optional pipeline components.
        
        Each parameter may be provided to override the default implementation; if a parameter is None, a sensible default instance is created and stored on the builder.
        
        Parameters:
            collector (EvidenceCollector | None): Collector used to gather evidence; defaults to a new EvidenceCollector() when None.
            claim_generator (ClaimGenerator | None): Generator that produces claims from evidence; defaults to a new ClaimGenerator() when None.
            risk_aggregator (RiskAggregator | None): Aggregator that derives risks from claims and evidence; defaults to a new RiskAggregator() when None.
            action_generator (ActionGenerator | None): Optional enricher that augments risks with actions; defaults to a new ActionGenerator() when None.
        """
    ...
    def build(self, fetcher: DataFetcher, run_pk: int) -> EvidenceRegistry:
        """
        Orchestrates evidence collection, claim generation, risk aggregation and optional action enrichment, and returns a populated EvidenceRegistry.
        
        Parameters:
            fetcher (DataFetcher): Data source used for collection and generation for the specified run.
            run_pk (int): Identifier for the collection run.
        
        Returns:
            EvidenceRegistry: Registry containing:
                - evidence: collected EvidenceItem objects,
                - claims: generated claims,
                - risks: aggregated (and possibly enriched) risks,
                - warning_count: total number of warnings observed during collection,
                - warning_details: metadata about warnings including:
                    - "total": total warning count,
                    - "budget_passed": whether warning budgets were respected,
                    - "counts": per-category warning counts,
                    - "budgets": loaded budget thresholds per category,
                    - "warnings": list of warnings with `category`, `source`, and `message`.
        
        Behavior notes:
            - Emits warnings for budget violations and for failures in claim generation, risk aggregation, or action enrichment.
            - If evidence collection fails, returns an empty EvidenceRegistry.
        """
        total_warnings = 0
    ...
    def persist(
        ...
        """
        Persist an EvidenceRegistry to DuckDB landing tables for a given collection run, replacing any existing rows for that run.
        
        Parameters:
            registry (EvidenceRegistry): Registry containing evidence, claims, risks, and warning_details to persist.
            conn (Any): DB connection exposing execute/executemany compatible with DuckDB parameter style.
            collection_run_id (str): Identifier used to delete existing rows and tag inserted records.
        
        Details:
            - Idempotent: deletes existing rows in lz_evidence, lz_claims, lz_risks, and lz_warnings for collection_run_id before inserting.
            - Inserts evidence, claims, and risks in bulk; claim and risk ID lists are stored as comma-joined strings.
            - Expects registry.warning_details["warnings"] to be a list of dicts with keys "category", "source", and "message"; each entry is inserted into lz_warnings.
            - On any exception the function emits a warning and does not raise.
        """
        try:
