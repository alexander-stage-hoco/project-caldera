...
def test_registry_docker_tools_match_docker_runner() -> None:
    """
    Verify docker_runner.py derives DOCKER_TOOLS by calling get_docker_tool_names() from tool_registry.
    
    Skips the test if scripts/docker_runner.py is not present. Asserts the runner imports from tool_registry, references get_docker_tool_names, and defines DOCKER_TOOLS.
    """
...
def test_registry_validate_metadata_matches_ingestion() -> None:
    """
    Check that each tool present in both TOOL_REGISTRY and TOOL_INGESTION_CONFIGS has the same `validate_metadata` value in both sources.
    
    Tools that appear in TOOL_INGESTION_CONFIGS but are missing from TOOL_REGISTRY are ignored; the test fails if any tool present in both collections has differing `validate_metadata` flags.
    """
