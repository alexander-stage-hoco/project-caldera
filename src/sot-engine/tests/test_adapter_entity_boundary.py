...
def _load_schema(conn: duckdb.DuckDBPyConnection) -> None:
    """
    Load and execute the SQL schema into the given DuckDB connection.
    
    Reads the persistence/schema.sql file (located two levels above this file) and executes its SQL statements on the provided DuckDB connection, creating the required database objects in that connection.
    """
...
def _create_layout_run(
    ...
    """
    Create and persist a layout run fixture stamped with the provided repository and run identifiers.
    
    Parameters:
        conn (duckdb.DuckDBPyConnection): Active DuckDB connection used by repositories.
        run_id (str): Identifier to stamp the layout payload's metadata `run_id`.
        repo_id (str): Identifier to stamp the layout payload's metadata `repo_id`.
        extra_files (list[dict] | None): Optional list of file objects to append to the layout payload's `data.files` list.
    
    Returns:
        int: Primary key of the persisted layout run.
    """
...
def _load_fixture(name: str) -> dict:
    """
    Load a test fixture JSON file from the persistence/fixtures directory and return it as a dict.
    
    Parameters:
        name (str): Base filename of the fixture (without the "_output.json" suffix).
    
    Returns:
        dict: Parsed JSON object contained in the fixture file.
    """
...
def _prep(fixture_name: str) -> dict:
    """
    Prepare a fixture payload by loading it and stamping repository and run identifiers.
    
    Parameters:
        fixture_name (str): Base name of the fixture (without "_output.json") to load.
    
    Returns:
        dict: The loaded fixture payload with `metadata.repo_id` set to REPO_ID and `metadata.run_id` set to RUN_ID.
    """
...
class TestSchemaValidButEntityFails:
    ...
    def test_scc_line_components_exceed_total(self, tmp_path: Path) -> None:
        """
        Verify SccAdapter rejects file records whose code + comment + blank fields sum to more than total lines even though each field individually passes JSON schema validation.
        
        Prepares an SCC fixture with lines=100, code=80, comment=30, and blank=20 and asserts that adapter.persist(payload) raises a ValueError with the message "quality validation failed".
        """
        conn = duckdb.connect(str(tmp_path / "test.duckdb"))
        _load_schema(conn)
        _create_layout_run(conn, RUN_ID, REPO_ID)
    ...
    def test_roslyn_zero_line_start_fails(self, tmp_path: Path) -> None:
        """
        Prepare a Roslyn payload with a violation whose `line_start` is 0 and verify that persisting it fails with a validation error.
        
        This test expects `RoslynAnalyzersAdapter.persist` to raise `ValueError` containing "validation failed" when a violation's `line_start` is set to 0.
        """
        conn = duckdb.connect(str(tmp_path / "test.duckdb"))
        _load_schema(conn)
        _create_layout_run(conn, RUN_ID, REPO_ID)
