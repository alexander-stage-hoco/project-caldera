select *
from {{ ref('mart_dependensee_project_coupling_hotspots') }}
where
    -- WHERE clause contract: total_coupling >= 3 OR coupling_zscore > 2.0 OR coupling_pattern != 'normal'
    not (total_coupling >= 3 or coupling_zscore > 2.0 or coupling_pattern != 'normal')
    ...
    or (coupling_risk = 'critical' and total_coupling < 10)
    or (coupling_risk = 'high' and (total_coupling < 5 or total_coupling >= 10))
    or (coupling_risk = 'medium' and (total_coupling < 3 or total_coupling >= 5))
    or (coupling_risk = 'low' and total_coupling >= 3)
    ...
    or (coupling_pattern = 'hub' and not (fan_in > 3 and fan_out > 3))
    or (coupling_pattern = 'god_project' and not (fan_in > 5 and fan_out <= 2))
    or (coupling_pattern = 'octopus' and not (fan_out > 5 and fan_in <= 2))
    or (coupling_pattern = 'normal' and (fan_in > 3 and fan_out > 3))
