-- Aggregates Trivy vulnerability and IaC misconfiguration counts for files directly in each directory (non-recursive)
...
vuln_metrics as (
    ...
    left join {{ source('lz', 'lz_trivy_targets') }} t
        on t.run_pk = fd.run_pk
        and t.file_id = fd.file_id
),
...
iac_metrics as (
    ...
    group by fd.run_pk, fd.layout_run_pk, fd.directory_id, fd.file_id
),
