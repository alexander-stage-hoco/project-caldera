-- Git blame authorship counts per directory (recursive - all files in subtree)
...
dir_hierarchy as (
    ...
    where
        -- Match directories that are prefixes of the file's directory
        gbs.relative_path like ld.relative_path || '%'
        or ld.relative_path = '.'
)
select
    run_pk,
