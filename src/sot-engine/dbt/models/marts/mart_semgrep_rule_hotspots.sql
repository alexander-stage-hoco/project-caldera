-- Semgrep rule hotspots: identify most impactful rules for code quality prioritization
...
with rule_metrics as (
    select
        ...
        count(*) filter (where upper(severity) = 'INFO') as severity_info,
        count(*) filter (where upper(severity) in ('CRITICAL', 'HIGH')) as severity_high_plus,
        -- Weighted severity score for ranking
        (count(*) filter (where upper(severity) = 'CRITICAL') * 5 +
         count(*) filter (where upper(severity) = 'HIGH') * 4 +
         count(*) filter (where upper(severity) = 'MEDIUM') * 3 +
         count(*) filter (where upper(severity) = 'LOW') * 2 +
         count(*) filter (where upper(severity) = 'INFO') * 1) as severity_score
...
ranked_rules as (
    select
        *,
        row_number() over (partition by run_pk order by smell_count desc) as smell_rank,
        row_number() over (partition by run_pk order by severity_critical desc, severity_high desc, smell_count desc) as severity_rank,
        row_number() over (partition by run_pk order by files_affected desc, smell_count desc) as spread_rank,
