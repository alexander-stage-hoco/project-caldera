-- Aggregates gitleaks secret counts across all files in directory subtrees (recursive)

with recursive gitleaks_runs as (
...
file_metrics as (
    ...
    join {{ ref('stg_gitleaks_secrets') }} m
        on m.run_pk = fa.gitleaks_run_pk
