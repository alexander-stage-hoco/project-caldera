...
category_totals as (
    select
        dd_category,
        sum(violation_count) as total_count,
        count(distinct directory_id) as dir_count_with_violations
    from violations
...
dir_category_agg as (
    select
        v.dd_category,
        v.directory_id,
        sum(v.violation_count) as dir_total,
        sum(case when v.severity = 'critical' then v.violation_count else 0 end) as sev_critical,
        sum(case when v.severity = 'high' then v.violation_count else 0 end) as sev_high,
        sum(case when v.severity = 'medium' then v.violation_count else 0 end) as sev_medium,
        sum(case when v.severity = 'low' then v.violation_count else 0 end) as sev_low,
        sum(case when v.severity = 'info' then v.violation_count else 0 end) as sev_info
    from violations v
    group by v.dd_category, v.directory_id
),
