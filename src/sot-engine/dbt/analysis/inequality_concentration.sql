...
-- Directories where top contributors dominate (high concentration)
-- Focus on top_10_pct_share and top_20_pct_share metrics
...
combined as (
    select 'scc' as tool, * from scc_concentration
    union all
    select 'lizard' as tool, * from lizard_concentration
    union all
    select 'semgrep' as tool, * from semgrep_concentration
    union all
    select 'roslyn' as tool, * from roslyn_concentration
    union all
    select 'sonarqube' as tool, * from sonarqube_concentration
),
ranked as (
    select
        *,
        row_number() over (
            ...
            order by top_20_pct_share desc, file_count desc, directory_path
        ) as rank_id
    from combined
)
select
    ...
    round(hoover_value, 3) as hoover_value,
    file_count,
    round(avg_value, 2) as avg_value,
    round(max_value, 2) as max_value,
    round(p95_value, 2) as p95_value,
    case
        when top_10_pct_share >= 0.5 then 'extreme'
        when top_20_pct_share >= 0.7 then 'high'
        else 'moderate'
