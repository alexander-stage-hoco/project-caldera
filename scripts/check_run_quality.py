...
def _check_warnings(warnings_path: Path) -> bool:
    """
    Check a warnings JSON file against its embedded per-category budgets.
    
    If the file is missing the check is skipped and considered passed.
    
    Parameters:
        warnings_path (Path): Path to the warnings.json file to validate.
    
    Returns:
        `true` if all category counts are within their budget limits or the check was skipped, `false` if any category exceeded its budget.
    """
...
def main() -> int:
    """
    Run quality gate checks against a DuckDB quality summary for a collection run and return an exit code.
    
    Parameters:
        None (this function parses command-line arguments):
            --db (str): Path to the DuckDB database file to read quality summaries from.
            --min-trust (int): Minimum acceptable trust score (0-100); defaults to 50.
            --collection-run-id (str, optional): Specific collection run ID to evaluate; if omitted the most recent run is used.
            --check-warnings (str, optional): Path to a warnings.json file to validate warning budgets; when provided the file is validated and failures affect the exit code.
    
    Returns:
        int: Exit code where `0` indicates all quality gates passed (or no gate applied when no summary is found), and `1` indicates one or more gates failed (including missing database).
    """
